...
def download_3rd_party_packages(current_dir):
    ...
    install_package_proc = subprocess.Popen(
        install_package_cmd, shell=False, stderr=subprocess.PIPE, stdout=subprocess.PIPE
    )
    ...
    if err:
        ...
        if err.__contains__("DEPRECATION: Python 2.7 will reach the end of its life"):
            ...
            print(
                "Error occurred while downloading 3rd party packages.Error: {}".format(
                    err
                )
            )
            return False
...
def transfer_3rd_party_pkg(sample_ta_name, support_python_version):
    ...
    package_destination_path = os.path.join(
        "test", "functional", "sample_ta", sample_ta_name, "bin", sample_ta_name.lower()
    )

    for package_name in os.listdir("package"):
        if support_python_version == "python3" and package_name in [
            "httplib2",
            "functools32",
            "concurrent",
        ]:
        ...
        transfer_package_cmd = [
            "cp",
            "-rf",
            os.path.join("package", package_name),
            package_destination_path,
        ]
        transfer_task_proc = subprocess.Popen(
            transfer_package_cmd,
            shell=False,
        ...
        if "" != transfer_task_err:
            print(
                "Error while transfering 3rd packages to the Sample TA '{}'. Error: {}".format(
                    sample_ta_name, transfer_task_err
                )
            )
            return False

    print(
        "3rd party packages transferred successfully to sample TA '{}'".format(
            sample_ta_name
        )
    )
    return True
...
def archive_ta(sample_ta_name, current_dir):
    ...
    archive_task_proc = subprocess.Popen(
    ...
    if "" != archive_task_err:
        print(
            "Error while archiving the Sample TA '{}'. Error: {}".format(
                sample_ta_name, archive_task_err
            )
        )
        return False
    else:
        print(
            "Archived the Sample TA '{}'. Archived file path: {}".format(
                sample_ta_name, os.path.join(current_dir, archive_file_name)
            )
        )
