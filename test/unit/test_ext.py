...
def test_splunk_xml():
    ...
    assert event1[0] == "<stream><event>" "<data>data1</data></event></stream>"
    event2 = splunk_xml(
        "data2",
        time=12345677,
        index="index_test",
    ...
    assert (
        event2[0] == "<stream><event><time>12345677.000</time><index>index_test"
...
def test_json_path():
    test_cases = [
        ({"foo": [{"baz": 1}, {"baz": 2}]}, "foo[*].baz", 2, [1, 2]),
        ('{"foo": [{"baz": 1}, {"baz": 2}]}', "foo[*].baz", 2, [1, 2]),
        ([{"baz": 1}, {"baz": 2}], "[*].baz", 2, [1, 2]),
        (
